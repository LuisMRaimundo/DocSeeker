...
import cv2  # Verify if actually used
...
class NLPEnhancer:
    """Enhance searches with NLP-based synonyms and fuzzy matching."""
    @staticmethod
    def expand_query(query):
        doc = nlp(query)
        synonyms = set()
        for token in doc:
            for syn in token.similarity(nlp(token.text)):
                synonyms.add(syn.text)
        return list(synonyms)

    @staticmethod
    def fuzzy_match(query, text):
        return fuzz.partial_ratio(query, text) > 80
...
class MetadataExtractor:
    ...
    def extract_metadata(filepath):
        ...
        metadata = {
            ...
            "Creation Date": datetime.datetime.fromtimestamp(file_stats.st_ctime).strftime("%Y-%m-%d %H:%M:%S"),
            "Modification Date": datetime.datetime.fromtimestamp(file_stats.st_mtime).strftime("%Y-%m-%d %H:%M:%S"),
        }
        if filepath.endswith(".pdf"):
            ...
            doc = Document(filepath)
            metadata["Author"] = getattr(doc.core_properties, "author", "Unknown")  # Prevent errors
        return metadata

# ✅ Web API Route (Defined **after** app initialization)
@app.route("/search", methods=["POST"])
def search():
    """Handle search requests via Web interface"""
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            filepath = os.path.join(root, file)
            if any(filepath.endswith(ext) for ext in file_types):
                try:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        mode = sys.argv[1].lower()  # Get mode from command line
    else:
        mode = input("Run in GUI mode (Tkinter) or Web mode (Flask)? [GUI/Web]: ").strip().lower()
